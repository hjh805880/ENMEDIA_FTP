    ...
    <div class="shadow-black-800/60 flex flex-col items-center justify-between w-full max-w-5xl bg-white shadow-lg">
      <div class="bg-primary-500 flex items-center justify-between w-full px-8 py-5 text-white">
        <div class="rowCenter space-x-8">
          <div class="rowCenter space-x-4">
            ...
            <span class="font-light">{{ item.titleSub }}</span>
          </div>
          <div class="rowCenter space-x-2">
            <div v-for="(workItem, index) in item.work" :key="`work-${index}`" class="text-primary-500 px-4 py-1 text-sm font-medium bg-white rounded-full">
              {{ workItem }}
            </div>
          </div>
        </div>
      ...
      <div class="py-14 flex items-center justify-center w-full px-16 space-x-12">
        <div class="max-w-[40%] size-full aspect-1 overflow-hidden rounded-full colCenter shadow-inner">
          <NuxtImg format="webp" quality="80" width="1200px" v-zoom :src="`/img/portfolio/${item.titleImg}`" :alt="`이엔미디어 포트폴리오 업체 ${item.title} 타이틀 이미지`" class="hover:scale-125 object-cover z-[999999] w-full h-full transition-all duration-500 ease-in-out shadow-inner" />
        </div>
        <div class="flex flex-col w-full max-w-[60%] space-y-6 items-start justify-center">
          <div class="flex items-center justify-start w-full space-x-4">
            <NuxtImg :src="`/img/portfolio/${item.logo}`" :alt="`이엔미디어 포트폴리오 업체 ${item.title} 로고`" class="w-14" />
            <div class="text-black-800 w-fit text-3xl font-bold">{{ item.title }}</div>
            <div class="text-black-500 w-fit text-base font-light">{{ item.titleSub }}</div>
          </div>
          ...
          <div class="grid items-start justify-start w-full grid-cols-3 gap-2">
            ...
            <span class="text-black-700 col-span-2 font-light text-left">{{ item.type }}</span>
            ...
            <div class="col-span-2">
              <NuxtLink :to="`${item.homepage}`" target="_blank" class="has-[:hover]:text-primary-500 has-[:hover]:underline-offset-4 has-[:hover]:underline flex w-fit items-center text-black-700 justify-start space-x-1">
                <span class="font-semibold text-left">{{ item.homepage }}</span>
            ...
            <span class="text-black-700 col-span-2 font-light text-left">{{ item.media }}</span>
            <div class="grid items-center justify-start w-full grid-cols-4 col-span-3 gap-2 my-2 text-center">
              <div v-for="(historyItem, index) in item.history" :key="`history-${index}`" class="text-black-500 border-black-500 hover:text-primary-500 hover:border-primary-500 px-2 py-1 text-xs font-medium border rounded-full">
                {{ historyItem }}
              </div>
            </div>
            <span class="text-black-800 text-lg font-bold text-left"><b class="text-primary-500">-</b> TARGET</span>
            <span class="text-black-700 col-span-2 font-light text-left">{{ item.target }}</span>
            <span class="text-black-800 text-lg font-bold text-left"><b class="text-primary-500">-</b> STRATEGY</span>
            <span class="text-black-700 col-span-2 font-light text-left">{{ item.strategy }}</span>
          </div>
        </div>
      </div>
    </div>
