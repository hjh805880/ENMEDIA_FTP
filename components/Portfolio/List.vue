...
<template>
  <div class="max-w-gui w-full h-full pb-56 mx-auto space-y-24" :style="isLastItemOffset ? 'padding-bottom: 14rem;' : ''">
    <div data-aos="fade-up" data-aos-duration="1000" v-if="showCategoryButtons" class="rowCenter category-buttons space-x-6 text-xl font-bold">
      ...
      <button @click="setCategory('요식업')" class="text-black-700 px-6 py-1 rounded-full" :class="{ 'text-primary-500 bg-white border-2 border-white': selectedCategory === '요식업', 'bg-transparent text-white border-2 border-white': selectedCategory !== '요식업' }">요식업</button>
      <button @click="setCategory('쇼핑몰')" class="text-black-700 px-6 py-1 rounded-full" :class="{ 'text-primary-500 bg-white border-2 border-white': selectedCategory === '쇼핑몰', 'bg-transparent text-white border-2 border-white': selectedCategory !== '쇼핑몰' }">쇼핑몰</button>
      <button @click="setCategory('병원/의료업')" class="text-black-700 px-6 py-1 rounded-full" :class="{ 'text-primary-500 bg-white border-2 border-white': selectedCategory === '병원/의료업', 'bg-transparent text-white border-2 border-white': selectedCategory !== '병원/의료업' }">병원/의료업</button>
      <button @click="setCategory('삼성화재')" class="text-black-700 px-6 py-1 rounded-full" :class="{ 'text-primary-500 bg-white border-2 border-white': selectedCategory === '삼성화재', 'bg-transparent text-white border-2 border-white': selectedCategory !== '삼성화재' }">삼성화재</button>
        ...
        <a @click.prevent="openModal(item)">
          <div class="colCenter w-full space-y-6">
            <div class="lg:h-32 colCenter w-full overflow-hidden">
              <NuxtImg format="webp" width="1200px" :src="`/img/portfolio/${item.titleImg}`" :alt="`이엔미디어 포트폴리오 업체 ${item.title} 썸네일 이미지`" class="group-hover/portfolio:scale-100 w-full transition-all duration-700 ease-in-out scale-150" />
            </div>
    ...
    <div class="shadow-black-800/60 flex flex-col items-center justify-between w-full max-w-5xl bg-white shadow-lg">
      ...
      <div class="py-14 flex items-center justify-center w-full px-16 space-x-12">
        <div class="max-w-[40%] size-full aspect-1 overflow-hidden rounded-full colCenter shadow-inner">
          <NuxtImg format="webp" width="1200px" v-zoom :src="`/img/portfolio/${selectedItem.titleImg}`" :alt="`이엔미디어 포트폴리오 업체 ${selectedItem.title} 타이틀 이미지`" class="hover:scale-125 object-cover z-[999999] w-full h-full transition-all duration-500 ease-in-out shadow-inner" />
        </div>
